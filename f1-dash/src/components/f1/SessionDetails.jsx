...
export default function SessionList({sessionKey, positionRange}) {
    ...
    return (<div className="space-y-3">
            {Object.entries(grouped).map(([country, countrySessions]) => (
                <div key={country} className="border rounded-lg dark:border-gray-700">
                    {/* Country header */}
                    <button onClick={() => setOpenCountry(openCountry === country ? null : country)}
                            className="w-full text-left px-4 py-3 font-medium bg-gray-100 text-gray-300 dark:bg-gray-800">
                        {country}
                    </button>
                    {openCountry === country && (<div className="p-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                            {countrySessions.map((s) => (<button
                                    key={s.session_key}
